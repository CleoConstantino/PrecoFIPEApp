package com.github.precofipeapp.util;

public class KeyUtil {

    public static final String KEY_REGISTRO_NOME_USUARIO = "KEY_REGISTRO_NOME_USUARIO";
    public static final String KEY_REGISTRO_EMAIL_USUARIO = "KEY_REGISTRO_EMAIL_USUARIO";
    public static final String KEY_REGISTRO_SENHA_USUARIO = "KEY_REGISTRO_SENHA_USUARIO";
    public static final String KEY_USUARIO = "KEY_USUARIO";
    public static final String KEY_SENHA = "KEY_SENHA";
    public static final String KEY_CODIGO_MARCA = "KEY_CODIGO_MARCA";
    public static final String KEY_CODIGO_MODELO = "KEY_CODIGO_MODELO";
}
